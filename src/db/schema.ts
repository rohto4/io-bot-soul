...
async function seedRuntimeSettings(db: DbClient, now: string): Promise<void> {
  const settings = [
    ...
    ["NOTES_PER_HOUR", "5", "integer", "rate_limit", "1時間あたりの最大投稿数。超過時はscheduled-post.tsでskip。"],
    ["NOTES_PER_DAY", "50", "integer", "rate_limit", "1日あたりの最大投稿数。超過時はscheduled-post.tsでskip。"],
    ["QUOTE_RENOTES_PER_DAY", "5", "integer", "rate_limit", "1日あたりの最大引用RN数。超過時はscheduled-post.tsでskip。"],
    ...
    ["EXPERIENCE_CANDIDATE_POST_PROBABILITY", "0.10", "number", "experience", "5分tickで体験候補投稿ガチャに入る確率。候補がなければskip。"],
