...
flowchart TD
  ...
  H[GitHub Actions Scheduled Post Draw] --> I[npm run scheduled:post-draw:prod]
  I --> J[src/scheduled.ts post-draw]
  J --> K[5分ごとの投稿抽選]
  K --> L[通常ノート投稿またはskip]
...
|---|---|---|---|---|
| `npm start` | `node dist/main.js` | Docker常駐 | 毎分polling、フォロー返し、リプライ返信、❤同意確認 | 返信、フォロー案内 |
...
| `npm run scheduled:post-draw:prod` | `node dist/scheduled.js post-draw` | GitHub Actions / Docker手動 | build済みJSで投稿抽選を1回だけ実行 | 通常ノート |
| `npm run db:migrate` | `tsx src/db/migrate.ts` | 手動 / 初期化 | DB schema作成とseed | なし |
| `npm test` | `vitest run` | 手動 / CI候補 | テスト実行 | なし |
| `npm run build` | `tsc -p tsconfig.json` | 手動 / Actions | TypeScript build | なし |

## Docker常駐アプリ

Docker常駐アプリは、Misskey通知に反応するためのプロセス。
通常ノートの定期投稿抽選は行わない。
...
これらはGitHub Actionsまたは今後追加する別スクリプトで扱う。

## GitHub Actions投稿抽選

`Scheduled Post Draw` は、5分ごとに起動する投稿抽選workflow。
Actionsが起動することと、投稿されることは別。

```mermaid
flowchart TD
  A[5分ごとのschedule] --> B[npm run scheduled:post-draw:prod]
...
secretはこの仕組みに載せない。
`MISSKEY_TOKEN`、`DATABASE_URL`、`CHUTES_API_KEY`、`OPENAI_API_KEY` は引き続き `.env.local` またはGitHub secretsで管理する。
