...
- **環境変数**: Dockerの起動時に読み込む。変更時は `docker compose restart bot` が必要。
- **DBマスタ**: 起動中のまま変更できる。最大5分（次のtick）で反映。再起動不要。
...
| 変数名 | 初期値 | 説明 |
|---|---|---|
| `SCHEDULED_POSTING_ENABLED` | `false` | **投稿の最終安全スイッチ。** `true` にして初めて実投稿される。問題発生時はここを `false` に戻して `docker compose restart bot` |

### Misskey接続

| 変数名 | 必須 | 説明 |
|---|---|---|
| `MISSKEY_TOKEN` | ✅ | Misskey API トークン（secret） |
| `MISSKEY_HOST` | - | デフォルト: `https://misskey.io` |
| `PINNED_CONSENT_NOTE_ID` | - | ピン留め同意ノートのID（❤リアクション監視対象） |
| `ADMIN_ACCOUNT` | - | デフォルト: `@unibell4` |

### AI APIキー（secret）

| 変数名 | 説明 |
|---|---|
| `CHUTES_API_KEY` | Chutes API認証キー（primary AI） |
| `OPENAI_API_KEY` | OpenAI APIキー（fallback AI） |

### DB接続

| 変数名 | デフォルト | 説明 |
|---|---|---|
| `DATABASE_PROVIDER` | `sqlite` | `sqlite` または `postgres` |
| `DATABASE_URL` | - | Postgres接続文字列（`DATABASE_PROVIDER=postgres` 時） |
| `SQLITE_PATH` | `./data/bot.sqlite` | SQLiteファイルパス |

### タイマー間隔（env varのみ、DB管理ではない）

| 変数名 | デフォルト | 説明 |
|---|---|---|
| `POLL_INTERVAL_SECONDS` | `60` | フォロー・リプライ・❤確認の間隔（秒） |
| `POST_DRAW_INTERVAL_SECONDS` | `300` | 投稿抽選の間隔（秒）= 5分 |
| `EXPERIENCE_SCAN_INTERVAL_SECONDS` | `600` | 体験候補蓄積の間隔（秒）= 10分 |
...
## DBマスタ（`m_runtime_setting`）設定値の確認・変更
...
| **雰囲気言及比率** | 0.75 | `TL_VIBE_RATIO` | TL参照時の雰囲気言及の比率（残り25%が自動的に特定言及） |

> **注意**: `TL_MENTION_RATIO` はDBキーとして独立していない。コードでは `1 - TL_VIBE_RATIO` として計算される。
...
タイマー間隔は env var で設定するが、投稿判定の最短間隔はDBで調整可能。
...
| 本文生成モデル | gpt-5.4-mini | `OPENAI_MODEL_TEXT` |
| 分類モデル | gpt-5.4-mini | `OPENAI_MODEL_CLASSIFIER` |
...
## 画像・アセット設定（Phase 7 / P1）

| 設定 | 初期値 | DBキー | 説明 |
|---|---|---|---|
| 画像の標準cooldown | 24時間 | `EMOTION_ASSET_DEFAULT_COOLDOWN_HOURS` | 同じ画像の連続使用を防ぐ間隔 |
